...
const getModel = getBuiltinModel as unknown as (provider: string, id: string) =>
  Model<Api> | undefined;
...
export function resolveModel(id: string): PiModel | undefined {
  ...
  if (entry?.custom) {
    const template = builtinModel(entry.custom.template);
    return template
      ? cloneModel(template, id, entry.name, { baseUrl: entry.custom.baseUrl })
      : undefined;
  }
